...
from .pyvesync_helpers import Helpers
from .pyvesync_helpers import logger as helper_logger
from .pyvesync_basedevice import VeSyncBaseDevice
from .pyvesync_bulb import *   # noqa: F403, F401
from .pyvesync_bulb import logger as bulb_logger
from .pyvesync_fan import *   # noqa: F403, F401
from .pyvesync_fan import logger as fan_logger
from .pyvesync_outlet import *   # noqa: F403, F401
from .pyvesync_outlet import logger as outlet_logger
from .pyvesync_kitchen import logger as kitchen_logger
from .pyvesync_kitchen import *  # noqa: F403, F401
from .pyvesync_switch import logger as switch_logger
from .pyvesync_switch import *  # noqa: F403, F401
...
def object_factory(dev_type, config, manager) -> Tuple[str, VeSyncBaseDevice]:
    ...
    Note:
        ...
        See [pyvesync_bulb.bulb_mods], [pyvesync_fan.fan_mods],
        [pyvesync_outlet.outlet_mods], [pyvesync_switch.switch_mods],
        and [pyvesync_kitchen.kitchen_mods] for more information.
    """
    def fans(dev_type, config, manager):
        fan_cls = fan_modules[dev_type]  # noqa: F405
        return 'fans', fan_cls(config, manager)

    def outlets(dev_type, config, manager):
        outlet_cls = outlet_modules[dev_type]  # noqa: F405
        return 'outlets', outlet_cls(config, manager)

    def switches(dev_type, config, manager):
        switch_cls = switch_modules[dev_type]  # noqa: F405
        return 'switches', switch_cls(config, manager)

    def bulbs(dev_type, config, manager):
        bulb_cls = bulb_modules[dev_type]  # noqa: F405
        return 'bulbs', bulb_cls(config, manager)

    def kitchen(dev_type, config, manager):
        kitchen_cls = kitchen_modules[dev_type]
        return 'kitchen', kitchen_cls(config, manager)
...
class VeSync:  # pylint: disable=function-redefined
    ...
    def debug(self, new_flag: bool) -> None:
        """Set debug flag."""
        log_modules = [bulb_logger, switch_logger, outlet_logger, fan_logger, helper_logger]
        if new_flag:
            logger.setLevel(logging.DEBUG)
            for m in log_modules:
                m.setLevel(logging.DEBUG)
